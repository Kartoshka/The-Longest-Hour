...
public class GeometryHelper
{
	...
	public static Vector3 calculateCentroidPosition(List<Vector3> points)
	{
        ...
        public static Vector3 calculateCentroidPosition(Vector3[] points)
        {
            Vector3 centroid = Vector3.zero;
        ...
        /// <summary>
        /// See if point is contained inside a polygon. Ignores Y.
        /// </summary>
        ...
        public static float getLongestDistanceInPoly(Vector3[] polyPoints)
        {
            float longest = -1;
            for(int i = 0; i < polyPoints.Length; i++)
            {
                for(int j = i+1; j < polyPoints.Length; j++)
                {
                    float dist = Mathf.Abs(Vector3.Distance(polyPoints[i], polyPoints[j]));
